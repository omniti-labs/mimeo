...
ALTER TABLE @extschema@.refresh_config_dml DROP CONSTRAINT IF EXISTS refresh_config_dml_source_table_unique;
ALTER TABLE @extschema@.refresh_config_logdel DROP CONSTRAINT IF EXISTS refresh_config_logdel_source_table_unique;
...
DROP FUNCTION IF EXISTS @extschema@.run_refresh(text, int, boolean);
...
LOOP
    ...
    PERFORM dblink_exec('mimeo_logdel', 'GRANT INSERT ON TABLE '||v_source_queue_table||' TO '||v_row.grantee);
    PERFORM dblink_exec('mimeo_logdel', 'GRANT EXECUTE ON FUNCTION '||v_source_queue_function||' TO '||v_row.grantee);
END LOOP;
PERFORM gdb(p_debug, 'v_create_trig: '||v_create_trig); 
